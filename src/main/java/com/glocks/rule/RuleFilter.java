...
public class RuleFilter {
    ...
    public HashMap getMyRule(Connection conn, HashMap<String, String> device_info, ArrayList<Rule> rulelist) {
        ...
        for (Rule rule : rulelist) {
            ...
            try {
                RuleInfo re = new RuleInfo(edrappdbName, appdbName, auddbName, repdbName, device_info.get("rule_name"), "executeRule", "EDR", device_info.get("IMEI"), "0", device_info.get("IMEI").length() > 14 ? device_info.get("IMEI").substring(0, 14) : device_info.get("IMEI"),
            ...
            if (device_info.get("output").equalsIgnoreCase(output)) {
                ...
                RuleInfo re = new RuleInfo(edrappdbName, appdbName, auddbName, repdbName, device_info.get("rule_name"), "executeAction", "EDR", device_info.get("IMEI"), "0", device_info.get("file_name"),
